...
class SFGA(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: int = 7, reduction: int = 16, residual: bool = True):
        """Initialize SFGA with channel, spatial, and local edge-guided attention branches."""
        super().__init__()
        if k not in (3, 5, 7, 9):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply smoke-flame guided residual feature recalibration."""
        x = self.proj(x)
...
class C3k2SFGA(C3k2):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass with smoke-flame guided feature recalibration."""
