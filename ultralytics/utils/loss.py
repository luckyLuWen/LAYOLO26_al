...
class v8DetectionLoss:
    ...
    @staticmethod
    def _parse_class_ids(ids) -> list[int]:
        """Parse class IDs from a string, integer, or iterable."""
        if ids is None:
            return []
        if isinstance(ids, str):
            ...
            if not ids:
                return []
            return [int(x) for x in ids.replace(" ", "").split(",") if x != ""]
    ...
    def __init__(
        ...
        self.nofire_class_ids = [
            i for i in self._parse_class_ids(getattr(h, "nofire_class_ids", "")) if 0 <= i < self.nc
        ]
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        bce_loss = self.bce(pred_scores, cls_target)  # (bs, num_anchors, nc)
        if self.fl_gamma > 0:
            ...
            if self.fl_alpha > 0:
                alpha = torch.as_tensor(self.fl_alpha, device=pred_scores.device, dtype=dtype)
                alpha_weight = cls_target * alpha + (1.0 - cls_target) * (1.0 - alpha)
        ...
        # Fire-state priority loss for emergency risk semantics.
        if self.fire_prior > 0 and fg_mask.sum() and self.fire_class_ids and self.nofire_class_ids:
            ...
            fire_ids = torch.as_tensor(self.fire_class_ids, device=pred_scores.device, dtype=torch.long)
            nofire_ids = torch.as_tensor(self.nofire_class_ids, device=pred_scores.device, dtype=torch.long)
